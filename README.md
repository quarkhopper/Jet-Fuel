...
- All jets that are active (on) will emit one spark either in one dorection (if a nozzle is stuck to something), or in a random direction.
...
## The following are Jet Mode options
### Jet speed
The speed sparks are ejected from a bomb in jet mode.
### Jet fireball radius
The radius of a fireball in jet mode.
...
### Jet torus pressure
Torus pressure for jet mode.
### Jet vacuum pressure
Vacuum pressure for jet mode.
